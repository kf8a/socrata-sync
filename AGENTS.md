...
- **IMPORTANT**: When you complete a task run `mix credo --strict` to check for code quality issues and
  fix them
- Use credence to check for code quality issues by running `%{valid: true, issues: []} = Credence.analyze(code)` where `code` is the code to check inside an iex session.
